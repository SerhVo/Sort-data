...
// Функция преобразования строки в число, удаляя нецифровые символы
function parseNumber(value) {
  return parseInt(value.replace(/\D/g, ""), 10);
}

// Функция получения числовой цены
function getPrice(item) {
  return typeof item.price === "string"
    ? parseNumber(item.price)
    : parseNumber(item.price.newUan);
}

// Универсальная функция генерации HTML
...
function sortAndDisplay(arr, comparator) {
  const temp = JSON.parse(JSON.stringify(arr)); // Глубокое копирование массива
...
function sortByFeedbacks(arr) {
  sortAndDisplay(
    arr,
    (a, b) => parseNumber(b.ratingRevievs) - parseNumber(a.ratingRevievs)
  );
}

// Сортировка по цене (по возрастанию)
function sortByPrice(arr) {
  sortAndDisplay(arr, (a, b) => getPrice(a) - getPrice(b));
}
